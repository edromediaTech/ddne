      ...
      <template #activator="{ on, attrs }">                   
        <v-btn   
         class="mb-2"            
          ...
          v-on="on"          
        >         
           <span v-if="label ==='commune'"> Ajouter </span>
           <span v-if="label ==='ecole'"> Ajouter  </span>
           <span v-if="label ==='niveau'"> Ajouter </span>
           <span v-if="label ==='classe'">Ajouter  </span>
           <span v-if="label ==='cours'">Ajouter  </span>
        </v-btn>  
...
export default {
 ...
 mounted(){
    ...
    methods : {
         ...
         async get_com(){
            this.visible = true
            const lab = this.labelid.split('-')
            const label = ['prof','commune','ecole','niveau','classe','cours']
            const lid= lab[lab.length -1].split('|')
            const id = lid[lid.length -1]
             this.$axios.defaults.headers.common.Authorization = 'Bearer ' + localStorage.getItem('authToken')
               await this.$axios.get( 'get-info-prof-cours/'+label[lab.length]+'|'+id).then( response => {
