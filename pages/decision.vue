<template>
  ...
  <v-data-table :headers="headers" :items="eleves"  :search="search" :footer-props="{'items-per-page-options':[50, 100, -1]}">
    <template #top>
...
import tablePrintDecision from '~/components/tablePrintDecision.vue';
    ...
    methods: {
       generateReport () {
           this.visible = true           
            ...
            this.infoEcole =  localStorage.ecole +' - '+ (this.classes.find(el => el.value === this.classe)).text
  ...
  get_decision (){    
            this.visible = true
             this.$axios.get('get-decision/' + localStorage.ecole_id+'|'+this.classe+'|'+ localStorage.anac).then((response)=>{
                 console.log(response.data);
