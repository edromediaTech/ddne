...
export default {
    ...
    data (){
        ...
        // },
            ...
            users: [],
            privileges:[{text: 'User', value:0},{text: 'Operateur', value:1}, {text: 'Directeur', value:2},
            {text: 'Adm', value:6}, {text: 'Iz', value:3},{text: 'Ip', value:5}, {text: 'Ass Compt', value:10},
            {text: 'comptable', value:11}, {text: 'Comptable Adm', value:12}, {text:'Super', value:4}],
            privilege:['User', 'Operateur', 'Directeur', 'Iz', 'Sup', 'Ip', 'Adm', 'Ass Compt', 'Comptable', 'Comptable Adm'],
            headers: [
  ...
  methods : {
     ...
     async  get_users_online () {
          this.$axios.defaults.headers.common.Authorization = 'Bearer ' + localStorage.getItem('authToken')
              ...
              this.visible = true  
          await this.$axios.get('online-user').then(res => {
              this.users = res.data
 ...
 updateUser (user) {  
   this.$axios.defaults.headers.common.Authorization = 'Bearer ' + localStorage.getItem('authToken')
        ...
        this.$axios.patch('edit-user/' + user.id, this.editedItem).then(res => {
             if(res.data === 1)
 ...
 updatePrivilege (user) {  
   this.$axios.defaults.headers.common.Authorization = 'Bearer ' + localStorage.getItem('authToken')                
        this.$axios.patch('user-level/'+ user.id+'|'+ user.user_level).then(res => {
           this.$notifier.showMessage({ content: 'Privilège modifié', color: 'success' })
